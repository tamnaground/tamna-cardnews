---
name: 제주어영상
description: 탐라그라운드(@tamnaground) 제주어(제주 방언) 콘텐츠의 제작·수정 스킬. 다음 요청이면 작업 시작 전에 반드시 이 스킬을 사용할 것 — 제주어 영상/쇼츠/릴스/카드뉴스 영상의 신규 제작, 기존 영상의 대사·자막·음성·장면 수정이나 재렌더링, 삼춘 대화 영상, 제주어 대사 작성이나 문법·표기 검증(영상 없이도), HeyGen 실사 영상 프롬프트 준비, 유튜브 제주어 콘텐츠 조사. 브랜드 규격(1080x1920 세로형)·팔레트·로컬 렌더링 파이프라인(espeak-ng+Playwright+ffmpeg)과 실제 겪은 버그 회피법이 정의되어 있음.
---
